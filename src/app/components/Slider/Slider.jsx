...
const Slider = () => {
  ...
  // Scroll left or right manually
  const scroll = (direction) => {
    if (sliderRef.current) {
      const scrollAmount = window.innerWidth < 768 ? 150 : 250; // Adjust based on screen size
  ...
  return (
    ...
    >
      <button className={styles.prev} onClick={() => scroll("left")}>
        &#10094;
      </button>

      <div className={styles.slider} ref={sliderRef}>
        <div className={styles.sliderTrack}>
          {/* Duplicate Vcards for infinite effect */}

          {kidcloth.map((link, index) => (
            <Vcard link={link} key={index}></Vcard>
          ))}
        </div>
      </div>

      <button className={styles.next} onClick={() => scroll("right")}>
        &#10095;
      </button>
    </div>
  );
